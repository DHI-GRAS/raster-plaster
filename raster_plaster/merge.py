"""modified version of rasterio.merge, does not reverse paint and feathers overlap between merges"""
...
def merge(datasets, bounds=None, res=None, precision=7, indexes=None,
    ...
    tuple
        ...
        Two elements:
            ...
            merged: numpy ndarray
                Result of merging next raster with result from previous iteration

            changes: numpy ndarray
                Pixels with value of 1 were changed since last iteration (unless they were nodata)
    ...
    for src in datasets:
        ...
        nodataval = src.nodata or np.nan
        ...
        temp = src.read(out_shape=temp_shape, window=src_window, resampling=resampling,
        ...
        if np.isnan(nodataval):
            ...
            temp_valid = ~np.isnan(temp)
        else:
            region_valid = region != nodataval
